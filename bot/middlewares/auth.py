...
from bot.core.config import REQUIRED_CHANNEL
...
def build_subscribe_keyboard(lang: str = "ru") -> InlineKeyboardMarkup:
...
class SubscriptionMiddleware(BaseMiddleware):
    """Проверяет подписку на канал для приватных чатов."""
    ...
    async def __call__(
        ...
        lang    = (await db.get_user_lang(session, user_id) if session else None) or "ru"
        bot     = data["bot"]
        ...
        if isinstance(event, Message):
            await event.answer(
                ...
                reply_markup=build_subscribe_keyboard(lang),
                parse_mode="HTML",
            )
