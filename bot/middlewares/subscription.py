...
def _build_subscribe_keyboard(lang: str = "ru") -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=LOCALIZATION[lang]["btn_subscribe_channel"], url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}")],
        [InlineKeyboardButton(text=LOCALIZATION[lang]["btn_check_subscription"], callback_data="check_subscription")],
    ])
...
async def _is_subscribed(bot, user_id: int) -> bool:
    ...
    try:
        ...
        logger.warning("Не удалось проверить подписку на канал %s.", REQUIRED_CHANNEL)
        return True
...
class SubscriptionMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if await _is_subscribed(bot, user_id):
            return await handler(event, data)
        if isinstance(event, Message):
            await event.answer(LOCALIZATION[lang]["subscription_not_subscribed"], reply_markup=_build_subscribe_keyboard(lang), parse_mode="HTML")
        elif isinstance(event, CallbackQuery):
            await event.answer(LOCALIZATION[lang]["subscription_not_done"], show_alert=True)
        return None
