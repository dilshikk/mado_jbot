...
async def set_language(message: Message, state: FSMContext, session: AsyncSession) -> None:
...
async def my_status(message: Message, lang: str, session: AsyncSession) -> None:
    try:
...
async def cmd_stats(message: Message, lang: str, session: AsyncSession) -> None:
    if message.from_user.id not in ADMIN_IDS:
        return
    total_users, total_apps = await db.get_stats(session)
    await message.answer(
        LOCALIZATION[lang]["hr_stats_text"].format(total_users=total_users, total_apps=total_apps),
        parse_mode="HTML",
    )


@router.message(Command("broadcast"))
async def cmd_broadcast(message: Message, lang: str, session: AsyncSession) -> None:
    if message.from_user.id not in ADMIN_IDS:
        return
    if not message.reply_to_message:
        await message.answer(LOCALIZATION[lang]["broadcast_no_reply"])
        return
    users = await db.get_all_user_ids(session)
    total = len(users)
    sent = failed = 0
        ...
        if i % 25 == 0 or i == total:
            with suppress(Exception):
                await progress.edit_text(
                    LOCALIZATION[lang]["broadcast_progress"].format(current=i, total=total)
                    + f"\n✅ {sent}  ❌ {failed}"
                )
        await asyncio.sleep(_BROADCAST_DELAY)
    await progress.edit_text(
        LOCALIZATION[lang]["broadcast_done"].format(sent=sent, failed=failed),
        parse_mode="HTML",
    )
