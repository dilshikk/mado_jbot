...
- График работы (inline — CallbackQuery)
- Вечерние смены
- Выходные и праздники
- Курение (inline — CallbackQuery)
- Медицинская книжка (inline — CallbackQuery)
- Фото кандидата
"""
...
async def _ask_position_after_languages(
    ...
    await message.answer(
        ...
        reply_markup=kb.get_positions_inline_keyboard(lang, vacancies),
        parse_mode="HTML",
    )
...
async def handle_salary(message: Message, state: FSMContext) -> None:
    ...
    if not _is_valid_salary(text):
        err = (
            "❌ Укажите сумму цифрами, например: 3 000 000 или 3 000 000 – 5 000 000 \n"
            "Или нажмите «⏭ Пропустить»."
            if lang == "ru" else
            "❌ Miqdorni raqamlar bilan yozing, masalan: 3 000 000 yoki 3 000 000 – 5 000 000 \n"
...
async def _ask_schedule(message: Message, state: FSMContext, lang: str) -> None:
    ...
    # ── Inline-клавиатура выбора графика ──
    await message.answer(_t(lang, "ask_schedule"), reply_markup=kb.get_schedule_inline_keyboard(lang))


# ─── 6. График работы (Inline CallbackQuery) ──────────────────────────────────
...
async def handle_schedule(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    lang = _lang(data)
    key = callback.data.split(":")[1]  # "schedule_6_1" etc or "skip"
    value = None if key == "skip" else _t(lang, key, key)
    await state.update_data(schedule=value)
    with suppress(TelegramAPIError):
        await callback.message.edit_reply_markup(reply_markup=None)
    await state.set_state(Form.waiting_evening_shifts)
    await callback.message.answer(_t(lang, "ask_evening_shifts"), reply_markup=kb.get_evening_shifts_keyboard(lang))
    logger.info("handle_schedule: user_id=%d schedule=%r", callback.from_user.id, value)
...
async def handle_weekends(message: Message, state: FSMContext) -> None:
    ...
    # ── Inline-клавиатура курения ──
    await message.answer(_t(lang, "ask_smoking"), reply_markup=kb.get_smoking_inline_keyboard(lang))
    logger.info("handle_weekends: user_id=%d", message.from_user.id)


# ─── 9. Курение (Inline CallbackQuery) ───────────────────────────────────────
...
async def handle_smoking(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    lang = _lang(data)
    key = callback.data.split(":")[1]  # "no", "yes", "skip"
    ...
    # ── Inline-клавиатура медкнижки ──
    await callback.message.answer(_t(lang, "ask_med_book"), reply_markup=kb.get_med_book_inline_keyboard(lang))
    logger.info("handle_smoking: user_id=%d smoking=%r", callback.from_user.id, value)


# ─── 10. Медицинская книжка (Inline CallbackQuery) ────────────────────────────
...
async def handle_med_book(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    lang = _lang(data)
    key = callback.data.split(":")[1]  # "yes", "no", "in_progress", "skip"
    ...
    logger.info("handle_med_book: user_id=%d med_book=%r", callback.from_user.id, value)
