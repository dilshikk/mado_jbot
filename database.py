...
def migrate_db() -> None:
    ...
    with _connect() as conn:
        existing = {
            row[1] for row in
            conn.execute("PRAGMA table_info(applications)").fetchall()
        }
...
def get_users_to_unblock() -> list[int]:
    try:
        with _connect() as conn:
            rows = conn.execute(
                "SELECT user_id FROM blacklist WHERE unblock_at <= ?",
    ...
    with _connect() as conn:
        cursor = conn.execute(
            ...
            INSERT INTO applications
                (user_id, name, birthday, phone, position, experience, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, 'pending', ?)
            """,
...
def get_applications_today() -> list[dict]:
    ...
    with _connect() as conn:
        rows = conn.execute(
            "SELECT * FROM applications WHERE created_at LIKE ? ORDER BY created_at DESC",
            (f"{today}%",),
        ).fetchall()
        return [dict(r) for r in rows]
...
def search_applications_by_name(query: str) -> list[dict]:
    with _connect() as conn:
        rows = conn.execute(
            "SELECT * FROM applications WHERE name LIKE ? ORDER BY created_at DESC LIMIT 20",
            (f"%{query}%",),
        ).fetchall()
        return [dict(r) for r in rows]
...
def mark_reminder_sent(application_id: int) -> None:
    with _connect() as conn:
        conn.execute(
            "UPDATE applications SET reminder_sent = 1 WHERE id = ?",
            (application_id,),
        )
...
def get_stale_pending_applications(days: int = 3) -> list[dict]:
    ...
    try:
        with _connect() as conn:
            rows = conn.execute(
                """
                SELECT a.id, a.user_id, u.lang
                FROM applications a
                JOIN users u ON u.user_id = a.user_id
                WHERE a.status = 'pending'
                  AND a.notified_pending = 0
                  AND a.created_at <= ?
                """,
                (threshold,),
            ).fetchall()
...
def mark_pending_notified(application_id: int) -> None:
    with _connect() as conn:
        conn.execute(
            "UPDATE applications SET notified_pending = 1 WHERE id = ?",
            (application_id,),
        )
...
def get_score_stats() -> dict:
    try:
        with _connect() as conn:
            row = conn.execute(
                "SELECT AVG(hr_score) as avg_score, COUNT(hr_score) as scored_count "
                "FROM applications WHERE hr_score IS NOT NULL"
            ).fetchone()
            return {
...
def get_detailed_score_stats() -> dict:
    try:
        with _connect() as conn:
            rows = conn.execute(
                "SELECT hr_score, COUNT(*) as cnt FROM applications "
                "WHERE hr_score IS NOT NULL GROUP BY hr_score"
            ).fetchall()
            distribution = {r["hr_score"]: r["cnt"] for r in rows}
            scored_count  = sum(distribution.values())
            avg_row       = conn.execute(
                "SELECT AVG(hr_score) FROM applications WHERE hr_score IS NOT NULL"
            ).fetchone()
            comments = conn.execute(
                "SELECT name, hr_score, hr_comment FROM applications "
                "WHERE hr_comment IS NOT NULL AND hr_comment != '' "
            ...
            return {
                ...
                "top_comments":  [dict(r) for r in comments],
            }
    except sqlite3.Error as e:
...
def get_dashboard_stats() -> dict:
    ...
    with _connect() as conn:
        ...
        statuses = {
            s: count("SELECT COUNT(*) FROM applications WHERE status = ?", (s,))
            for s in ("pending", "accepted", "rejected", "hold", "hired")
        }
        top_pos = conn.execute(
            "SELECT position, COUNT(*) as count FROM applications "
            "GROUP BY position ORDER BY count DESC LIMIT 5"
        ).fetchall()
        ...
        return {
            ...
            "interviews_planned": count(
                "SELECT COUNT(*) FROM applications "
                "WHERE interview_time IS NOT NULL AND status = 'accepted'"
            ),
            "interviews_today":   count(
                "SELECT COUNT(*) FROM applications "
                "WHERE interview_time LIKE ? AND status = 'accepted'",
                (f"{today}%",),
            ),
...
def get_weekly_trend() -> list[dict]:
    ...
    for i in range(6, -1, -1):
        ...
        result.append({"label": label, "count": counts_by_day.get(day, 0)})
    return result
