...
# Telegram разрешает ~30 сообщений/сек глобально и ~1 msg/сек на чат
_BROADCAST_DELAY   = 0.05   # 50ms = ~20 msg/sec — безопасно
_PROGRESS_INTERVAL = 25     # обновлять прогресс каждые N сообщений
...
def _preview_keyboard(has_url: bool) -> InlineKeyboardMarkup:
    ...
    rows.append([
        InlineKeyboardButton(text="✏️ Изменить ссылку", callback_data="broadcast:edit:url"),
    ])
...
async def broadcast_got_caption(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "<b>Шаг 3/4.</b> Отправьте URL-ссылку (начинается с https://).\n"
        "Или пропустите.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
...
async def broadcast_got_url(message: Message, state: FSMContext) -> None:
    ...
    if not url.startswith("http"):
        await message.answer(
            "❌ Ссылка должна начинаться с <code>https://</code>", parse_mode="HTML"
        )
        return
...
async def _show_preview(message: Message, state: FSMContext) -> None:
    ...
    if photo_id:
        ...
        await message.answer(
            caption, parse_mode="HTML", reply_markup=url_kb,
        )

    await message.answer(
        "Всё верно?",
        reply_markup=_preview_keyboard(has_url=bool(url)),
    )
...
async def broadcast_send(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    for i, user_id in enumerate(user_ids, 1):
        try:
            ...
            # Пользователь заблокировал бота — не логируем, это норма
            blocked += 1
    ...
    await progress_msg.edit_text(
        ...
        f"❌ Ошибок при отправке:  <b>{failed}</b>\n\n"
