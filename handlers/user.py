...
from filters.common import IsPrivateChat
...
async def about_restaurant(message: Message, lang: str) -> None:
    await message.answer(
        LOCALIZATION[lang]["about_text"], parse_mode="HTML"
    )
...
async def cmd_stats(message: Message, lang: str) -> None:
    ...
    await message.answer(
        LOCALIZATION[lang]["hr_stats_text"].format(
...
async def cmd_broadcast(message: Message, lang: str) -> None:
    ...
    progress = await message.answer(
        LOCALIZATION[lang]["broadcast_progress"].format(current=0, total=total)
    )
        ...
        if i % 25 == 0 or i == total:
            with suppress(Exception):
                await progress.edit_text(
                    LOCALIZATION[lang]["broadcast_progress"].format(
